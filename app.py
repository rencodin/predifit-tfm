...
if vista == "2️⃣ Análisis EDA":
    ...
    with col1:
        ...
        if ejercicio != "Todos":
            ...
            # Crear dos columnas para mostrar imágenes en paralelo
            col_img1, col_img2 = st.columns(2)

            # Imagen frontal
            ruta_frontal = f"assets/{ejercicio_archivo}_f.png"
            if os.path.exists(ruta_frontal):
                col_img1.image(Image.open(ruta_frontal), caption=f"{ejercicio} (frontal)")

            # Imagen trasera
            ruta_trasera = f"assets/{ejercicio_archivo}_t.png"
            if os.path.exists(ruta_trasera):
                col_img2.image(Image.open(ruta_trasera), caption=f"{ejercicio} (trasera)")

            # Imagen única sin sufijo
            ruta_simple = f"assets/{ejercicio_archivo}.png"
            if not os.path.exists(ruta_frontal) and not os.path.exists(ruta_trasera) and os.path.exists(ruta_simple):
                st.image(Image.open(ruta_simple), caption=f"{ejercicio}")

            # Si no hay ninguna imagen
            if not any(os.path.exists(path) for path in [ruta_frontal, ruta_trasera, ruta_simple]):
